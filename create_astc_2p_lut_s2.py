...
def generate_valid_2p_maps():
    ...
    print(f"Found {len(valid_maps_list)} unique, valid 2-partition maps (Expected 435).")

    return torch.tensor(valid_maps_list, dtype=torch.float32), \
           torch.tensor(valid_seeds_list, dtype=torch.int16)

def create_2p_ideal_to_seed_lut_s2(valid_maps_tensor, valid_seeds_tensor, device, batch_size=100000):
    total_ideal_maps = 2**16

    num_valid_maps = valid_maps_tensor.shape[0]
    print(f"Generating {total_ideal_maps} entries. S2 Invariance. {num_valid_maps} Valid Targets.")
...
lut_ideal_to_seed = create_2p_ideal_to_seed_lut_s2(valid_maps_tensor, valid_seeds_tensor, device, batch_size=100000)

file1 = "astc_2p_4x4_lut_s2.bin"
with open(file1, "wb") as f:
    # Only store first 2**5 entries since the second half are rotationally equivalent via (01)
    f.write(coalesce_lut_to_uint32(lut_ideal_to_seed[:1 + 2**15]).numpy().astype(np.uint32).tobytes())
